...
<div class="row">
    ...
    <form class="row g-3" action="{{ route('testimonials.store') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="col-md-6">
            ...
            <input type="number" id="start_count" name="start_count" value="{{ old('start_count') }}" class="form-control @error('start_count') is-invalid @enderror" min="1" max="5" required>
            @error('start_count') <small class="text-danger">{{ $message }}</small> @enderror
        </div>

        <div class="col-12">
            <label for="comment">Comment</label>
            <textarea id="comment" name="comment" rows="4" class="form-control @error('comment') is-invalid @enderror">{{ old('comment') }}</textarea>
